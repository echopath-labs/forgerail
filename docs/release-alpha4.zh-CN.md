...
相对 alpha.3 的三项兼容性收紧必须明确：contract 标识符至少包含两个字符且首字符必须为字母或数字；每个 Launch Contract 必须携带已验证的 `effectiveProfile`；每个 required active Pack 必须存在可用 manifest，并由 task envelope 显式包含。这些是 fail-closed 修正，不是未说明的兼容性回归。
