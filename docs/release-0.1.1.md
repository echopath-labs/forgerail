...
This patch corrects receipt observation, Profile resolution and approved binding writes. It retains the npm installation and explicit packaged-Skill loading scope of 0.1.0. This is a release candidate until the versioned GitHub Release and npm registry confirm publication.

- Validate every Profile candidate before selection; resolve equal-precedence rules independently of input order.
- Distinguish invalid workspaces, unavailable Git, non-Git directories and unborn Git branches. A self-reported v1 complete receipt returns valid=false and closeout=incomplete; it is not a completion certificate.
- Return CLI errors as JSON on stdout with a nonzero exit code. Consumers parsing stderr must migrate to the structured result.
- Preserve concurrent edits under the documented controlled single-writer model, retain recovery evidence on unsafe rollback, and preserve bytes outside managed blocks. Repeated unchanged writes are no-ops; malformed approved replacement markers are rejected.
...
The public candidate is an ordinary child of the observed remote `main`. Prepare branch `codex/forgerail-0.1.1` from that observed baseline through the source-owned projection. Any required correction is an ordinary source-first successor commit; do not rewrite approved history. The Draft PR base and publication comparison baseline remain bound to the observed remote `main` until revalidated against any concurrent changes.

Request an actual Agent bot review before merge, preferring Codex and otherwise the configured team bot. CI and self-review do not replace it. Resolve accepted findings and ensure the review covers the final head; if neither bot is available, ask the owner how to proceed. The merged public `main` tree must equal the final approved projection tree before creating annotated tag `v0.1.1`, publishing the exact package to npm `latest`, and creating the stable GitHub release.
