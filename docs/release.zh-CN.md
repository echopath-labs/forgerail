...
这是 ForgeRail 首个可用预发布版的项目专属 runbook，约束 `forgerail@0.1.0-alpha.1`、Git tag `v0.1.0-alpha.1` 与同一 EchoPath Labs Marketplace 快照；文档本身不授予任何执行权限。
...
4. 不暴露凭据地验证 npm 身份和 package ownership；确认 alpha.1 尚不存在，并记录 `latest`、`next`、`reserved`。
5. 仅以 public access 与 `next` tag 发布 `forgerail@0.1.0-alpha.1`；在可信发布链完成独立验证前关闭 provenance。
...
- 若精确 npm 发布成功但消费者 smoke 失败，只有得到明确批准后才可把可变 `latest`/`next` 恢复到最近可用版本。首个预发布版的恢复值为 `0.0.0-reserved.0`，并始终保留 `reserved`。
