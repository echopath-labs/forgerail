...
```text
请在当前项目工作区接入 https://github.com/echopath-labs/forgerail 的已发布 0.1.5 版，让后续工程任务能按项目规则使用它。先阅读官方安装与采用说明，检查当前 Agent 宿主、Node.js、已有接入、项目规则和 Git 状态，再选择该版本对当前宿主实际支持的方式。安装来源固定为 @echopath-labs/forgerail@0.1.5。若使用 Codex 项目接入，先展示只读计划并核对受管内容，在本次授权内执行，再做离线检查；其他宿主不要把显式加载说成自动发现。保留用户文件；遇到同名未知来源、漂移或规则冲突时，说明具体阻碍，不要强行覆盖。
