...
本文对应 `0.1.2` / `v0.1.2`。按 [npm 安装说明](installation.zh-CN.md)安装并显式加载包内 Skill。npm 安装不会注册原生 Plugin；若绑定要求原生发现，必须另外核验该依赖。
...
0.1.2 不启用机器消费的 ForgeRail 持久状态。只有重要证据无法通过项目现有来源表达，例如反复出现跨宿主冲突或确有机器强制策略需求时，才应考虑这一层。
...
| --- | --- | --- | --- |
| Codex | `AGENTS.md` | `supported` | 保留 registry 支持状态；0.1.2 原生激活未验证。须在新任务核对实际加载方式及获批绑定 |
