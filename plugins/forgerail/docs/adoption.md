...
This guide accompanies `0.1.3` / `v0.1.3` release. Follow the [npm installation guide](installation.md) and explicitly load the packaged Skills. npm installation does not register a native Plugin; any binding that requires native discovery must have that prerequisite verified separately.
...
npx --yes @echopath-labs/forgerail@0.1.3 adoption-plan --workspace . --selection all-detected

# Explicit subset chosen from the validated Host Adapter Registry.
npx --yes @echopath-labs/forgerail@0.1.3 adoption-plan --workspace . --host codex

# Every adapter in the current validated registry.
npx --yes @echopath-labs/forgerail@0.1.3 adoption-plan --workspace . --selection all-available
```
...
Persisted machine-consumed ForgeRail state is deferred beyond 0.1.3. It should be considered only when important evidence cannot be represented coherently through existing project sources, such as repeated cross-host conflicts or genuinely machine-enforced policy.
...
| --- | --- | --- | --- |
| Codex | `AGENTS.md` | `supported` | Registry status retained; 0.1.3 native activation is unverified. Verify the selected loading route and approved binding in a fresh task |
