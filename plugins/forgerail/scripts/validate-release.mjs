...
export function validateRelease() {
  ...
  record(
    ...
    packageJson.scripts?.prepublishOnly === "npm run test:maintainer"
      && ["npm test", "node scripts/forgerail.mjs validate-fixtures --scope full", "npm run test:integrity", "npm run test:shadow", "npm run test:release", "npm run test:consumer", "npm run test:directory"].every((command) => packageJson.scripts?.["test:maintainer"]?.split(" && ").includes(command)),
  ...
  const releaseEnglish = read("docs/release-alpha5.md");
  const releaseChinese = read("docs/release-alpha5.zh-CN.md");
  ...
  for (const phrase of [
    ...
    expectedTag,
    "Node.js 22 and 24",
    "codex/forgerail-alpha5-agw-replacement",
    "Do not unpublish",
    "AGW",
    "Host Binding Receipt",
    ".forgerail/",
