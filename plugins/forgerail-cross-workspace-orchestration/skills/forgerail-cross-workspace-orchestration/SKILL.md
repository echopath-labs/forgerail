...
- Keep `remote-integration-approval`, `release-approval`, `production-change-approval`, and `lifecycle-change-approval` independent and non-transitive. Every deploy work item must declare `deploymentEnvironment` as `development`, `preview`, `staging`, or `production`; only a production deployment requires both its release gate and its independent production-change gate.
